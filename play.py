...
parser = argparse.ArgumentParser(
    description="This program runs 2048 with given settings")
parser.add_argument(
    "-player", help="self = you play; computer = computer plays [default]")
parser.add_argument("-agent", help="which agent to use (see list)")
parser.add_argument("-iter", help="number of AI iterations")
parser.add_argument(
    "-graphics", help="whether to show graphics (AI Only). Any argument is True, default is False")
        ...
        if (args.agent not in gameAgentDict.keys()):
            ...
            if (args.iter != None):
                if (args.graphics != None):
                    runner = PerformanceTester(gameAgentDict[args.agent], iterations=int(
                        args.iter), graphics=bool(args.graphics))
                else:
                    runner = PerformanceTester(
                        gameAgentDict[args.agent], iterations=int(args.iter))
            else:
                if (args.graphics != None):
                    runner = PerformanceTester(
                        gameAgentDict[args.agent], graphics=bool(args.graphics))
                else:
